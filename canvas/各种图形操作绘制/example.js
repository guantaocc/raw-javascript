...
function updateRubberbandRectangle(loc) {
  ...
  rubberbandUlhc.x = loc.x > mousedown.x ? mousedown.x : loc.x;
  rubberbandUlhc.y = loc.y > mousedown.y ? mousedown.y : loc.y;
}
...
function drawRubberbandCircle(loc) {
  drawingContext.beginPath();
  let radius = powDistance(mousedown, loc);
...
function mouseMoveTouchEndInDrawingCanvas(loc) {
  ...
  if (dragging) {
    if (draggingControlPoint) {
      ...
      finishDrawingCurve();
      draggingControlPoint = false;
    } else if (selectedFunction === "erase") {
      // erase
      eraseLast();
    } else if (
      selectedFunction === "path" ||
...
iconCanvas.onmousedown = function (e) {
  let x = e.x || e.clientX,
    y = e.y || e.clientY;
  let loc = windowToCanvas(iconCanvas, x, y);
  e.preventDefault();
...
drawingCanvas.onmousemove = function (e) {
  let x = e.x || e.clientX,
    y = e.y || e.clientY;
  let loc = windowToCanvas(drawingCanvas, x, y);
  e.preventDefault();
...
drawingCanvas.onmouseup = function (e) {
  ...
  mouseMoveTouchEndInDrawingCanvas(loc);
};
...
drawGrid(drawingContext, GRID_LINE_COLOR, 10, 10);
selectedRect = ICON_RECTANGLES[LINE_ICON];
